...
3.3 결론 : Resource를 식별해야 한다면 PathVariable, 데이터의 결과값을 필터링 및 정렬 등을 해야 한다면 Query String을 선택하는 것이 일반적 
...
1. 사전 개념 학습 내용의 정리 방식
- 사전 개념 학습 내용도 Readme가 아닌 PPTX 형식으로 정리하여 제출해야 하는지 궁금합니다.
- 만약 정리해야 한다면 API 설계서와 동일한 파일에 섹션을 나누어 포함하는 것이 좋을지 아니면 별도의 학습 리포트 파일로 분리하는 것이 관리 측면에서 효율적일지 의견 여쭙습니다.

2. 실무 보고서 형식에 대한 피드백 요청
- 그동안은 학생 신분에서 제 기준 깔끔함 위주로 발표 자료를 만들어 왔으나 '실무 보고' 관점에서는 어떤 기준을 가져가야 할지 고민입니다.
- 예를 들어 정보 전달력을 높이기 위한 색감 활용 범위나 실무용 문체(개조식 보고 등) 등 멘토님이 보시기에 실무 환경에서 더 선호되는 작성 형식이 있다면 제 제출 자료에 대해 가감 없는 피드백을 부탁드리고 싶습니다.

3. API 설계 향후 방향성 점검
- 이번 주차에는 필수 데이터 위주로 설계를 마쳤으나 3주차 구현 전 다음과 같은 방향성으로 기능적 확장을 고려하고 있습니다. 이러한 방향성이 올바른지 궁금합니다.
- 조건별 기본값 설정: '부서별 사용자 수' API 호출 시 부서를 선택하지 않으면 Default로 전체 부서 정보를 응답하는 등의 유연성 부여
- 데이터 범위 확장: '게시글 작성 수'의 경우 공지사항 문의사항 등 게시판 타입별로 데이터를 확장하여 응답하는 구조
- 위와 같이 요구사항을 구체화하여 API의 완성도를 높이는 작업이 이번 과제의 의도와 잘 맞는지 확인 부탁드립니다.
